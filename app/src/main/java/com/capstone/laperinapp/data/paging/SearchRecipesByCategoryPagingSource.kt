...
class SearchRecipesByCategoryPagingSource(private val apiService: ApiService, val name: String): PagingSource<Int, DataItemRecipes>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, DataItemRecipes> {
        return try {
            val position = params.key ?: INITIAL_PAGE_INDEX
            val response = apiService.getRecipesByCategory(name, position, params.loadSize).data
